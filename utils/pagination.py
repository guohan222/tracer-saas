...
class Pagination(object):
    def __init__(self, current_page, all_count, base_url, query_params, per_page=2, pager_page_count=11):
        """
        分页初始化
    ...
    def page_html(self):
        ...
        if self.current_page <= 1:
            ...
            prev = '<li class="page-item"><a class="page-link" href="%s?%s">上一页</a></li>' % (self.base_url,
        ...
        for i in range(pager_start, pager_end + 1):
            ...
            if self.current_page == i:
                ...
                tpl = '<li class="page-item"><a class="page-link" href="%s?%s">%s</a></li>' % (self.base_url,
                                                                                               self.query_params.urlencode(),
                                                                                               i,)
        ...
        if self.current_page >= self.pager_count:
            ...
            self.query_params['page'] = self.current_page + 1
            nex = '<li class="page-item"><a class="page-link" href="%s?%s">下一页</a></li>' % (self.base_url,
                                                                                               self.query_params.urlencode(),)
        page_list.append(nex)

        if self.all_count:
            tpl = '<li class="page-item disabled"><span class="page-link">共%s条数据，页码%s/%s页</span></li>' % (
